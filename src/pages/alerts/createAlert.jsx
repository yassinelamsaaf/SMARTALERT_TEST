...
import { getNames, getModels, getCity, getSect,
    getSources, getChips, getSliders, sendData, getProduct } from "@/apis/mockAPI/addAlertsApi.js";
...
export default function CreateAlert() {
  ...
  const [steps, setSteps] = useState(["Essentials", "Chips", "Sliders", "Label"])
  ...
  const img = "/img/pages/alerts/to-right.png"
  // Chips & Sliders
  ...
  useEffect(() => {
    setStep(0);
    setForm({ brand: '', model: '', city: '', sector: '', source: '', chips: {}, sliders: {}, label: '' });
    setLoading(true);
    if(!productId) return
    Promise.all([
  ...
  return (
    ...
    <MetaComponent meta={metadata} />
      ...
      <div className="container" style={{maxWidth: 600, margin: '5vh auto'}}>
        <button className="btn btn-light" onClick={() => navigate("/alerts")}>{t[lang.lang||lang].alert.form.goBack}</button>
        <div className="alert-filter-card" style={{marginTop: 32}}>
          <h4 style={{marginBottom: '1.5rem'}}>{t[lang.lang || lang].alert.form.createTitle}</h4>
          {/* Progress Bar */}
          ...
          <div className="mb-3"><b>{t[lang.lang || lang].alert.form.step} {step + 1}:</b> {steps[step]}</div>
          <form onSubmit={handleSubmit}>
                ...
                {form.brand && form.brand.value && (
                  <>
                    ...
                    <select className="form-control" value={form.model?.value || ''} onChange={e => {
                      ...
                      {models?.length === 0 && <option value="">No options</option>}
                      {models?.map(m => (
                ...
                {form.city && form.city.value && (
                  <>
                    ...
                    <select className="form-control" value={form.sector?.value || ''} onChange={e => {
                      ...
                      {sectors.length === 0 && <option value="">No options</option>}
                      {sectors.map(s => (
                        <option key={s.value} value={s.value}>{s.label}</option>
                      ))}
                    </select>
              ...
              <div className="form-group">
                {chips.map(chip => { 
                  ...
                  <div key={chip.id} style={{ marginBottom: 12 }}>
                    <label>{t[usedLang].alert.info[chip.label] || chip.label}</label>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 6 }}>
                      {chip.valeurs.map(val => {
                        const langKey = typeof lang === 'string' ? lang : (lang?.lang || 'fr');
                        const translatedLabel = val.translations && val.translations[langKey]
                        ...
                        return (
                          <button
                            type="button"
                            key={val.value}
                            className={`btn ${form.chips[chip.label]?.value === val.value ? 'btn-primary' : 'btn-light'}`}
                            style={{ borderRadius: 20, padding: '4px 18px', fontWeight: 500 }}
                            onClick={() => handleChipChange(chip.id, chip.label, form.chips[chip.label]?.value === val.value ? '' : val.value)}
                          >
                            {translatedLabel}
                          </button>
                        );
            ...
            {step === 2 && (
              <div className="form-group">
                <label>{t[lang.lang || lang].alert?.form['sliders']|| 'Sliders'}</label>
                {sliders.map((slider, sliderIdx) => {
                  const sliderValue = form.sliders[slider.label]?.value || { min: slider.minValue, max: slider.maxValue };
                  return (
                    <div key={slider.id} style={{ marginBottom: 20 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 6 }}>
                        <input
                          ...
                          onChange={e => handleSliderChange(slider.id, slider.label, { ...sliderValue, min: Number(e.target.value) })}
                        ...
                        <input
                          ...
                          onChange={e => handleSliderChange(slider.id, slider.label, { ...sliderValue, max: Number(e.target.value) })}
                          className="form-control"
                          style={{ width: 90 }}
                        />
                      </div>
            ...
            {step === 3 && (
              <div className="form-group">
                ...
                <input className="form-control" value={form.label.value || ''} onChange={e => handleLabelChange(e.target.value)} required placeholder="Enter alert label" />
              </div>
            )}
            <div className="d-flex" style={{marginTop: 24, justifyContent: 'space-between'}}>
              {/* The Create Alert button remains for the last step */}
              {step === steps.length - 1 && (
                <button type="submit" className="btn btn-primary" disabled={loading}>{t[lang.lang || lang].alert.form.createTitle}</button>
              )}
            </div>
          </form>
