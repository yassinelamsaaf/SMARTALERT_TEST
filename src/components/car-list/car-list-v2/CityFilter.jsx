...
const CityFilter = ({ onCityFilterChange = (id) => { }, onFilterClear = () => { } }) => {
  ...
  const { criteria, loading, error } = useSelector((state) => state.cityCriteria);

  const handleOptionClick = (item) => {
    setSearchValue(item.name);
    onCityFilterChange(item.id)
  };
            ...
            >
              <div className="text-15 text-light-1 ls-2 lh-16">
                <input
                  autoComplete="on"
                  type="search"
                  placeholder={t[lang].sideBar.city.placeHolder}
